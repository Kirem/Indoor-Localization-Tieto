...
public class Searcher {
     ...
     *
     * @param query
     * @return list as pairs of objects where first element is name, second MapObject
     */
    public ArrayList<Pair<String,MapObject>> findElementsWithinQuery (String query){
        ArrayList <Pair<String,MapObject>> results = new ArrayList<>();
        for (Pair<String,MapObject> element: pairs){
                if(Pattern.compile(Pattern.quote(query), Pattern.CASE_INSENSITIVE).matcher(element.first).find()){
                    results.add(new Pair(element.first,element.second));
            }
        }
        return results;
    }
