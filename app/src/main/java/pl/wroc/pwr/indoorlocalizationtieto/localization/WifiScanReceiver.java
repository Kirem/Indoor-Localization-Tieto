...
/*
            if(scanCounter > 3) {
                ReferencePoint referencePoint = new ReferencePoint(
                        localization.getPosLat(),
                        localization.getPosLon(),
                        localization.getPosLevel()
                );

                int max = 0;
                for (ReferencePoint tempRefPoint : referencePointOccurences.keySet()) {
                    if(referencePointOccurences.get(tempRefPoint) > max) {
                        max = referencePointOccurences.get(tempRefPoint);
                        referencePoint = tempRefPoint;
                    }
                }
                updateLocation(referencePoint);
                referencePointOccurences = new HashMap<>();
                scanCounter = 0;
            }
*/
            ...
            textViewCounter.setText("Count: " + scanCounter);
            wifiManager.startScan();
        }
    }
